 ...
 */
public class PPDListNewProcessor implements PageProcessor {
    // 抓取网站相关配置
    ...
    public void process(Page page) {
        ...
        List<String> list = page.getHtml().xpath("//li[@class='clearfix']").all();
        if (list.size() <= 0) {
            // 忽略这个页面
            page.setSkip(true);
        }
...
//            System.out.println(tmp);
            System.out.println(tmp.xpath("//div[@class='w110 info']/a/i").toString().trim());
...
//        page.addTargetRequests(page.getHtml().links().regex("(https://www.zifangsky.cn/page/\\d*)").all());
    ...
    public static void main(String[] args) {
        Spider.create(new PPDListNewProcessor())
                .addUrl("http://invest.ppdai.com/loan/listNew")
                .thread(5)
                .run();
    }
}
